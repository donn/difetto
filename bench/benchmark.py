...
def run_test(tup):
    ...
    try:
        ...
        if chain == 'skip':
            skips += ["Difetto.Chain"]
            skips += ["OpenROAD.RepairDesign"]
            skips += ["Checker.DisconnectedPins"]
        elif chain == 'opt':
            opt_strat = True
        f = DifettoPNR(
            {
                "DESIGN_NAME": design_name,
                "VERILOG_FILES": [design_dir / "top.v"],
                "CLOCK_PORT": "CK",
                "CLOCK_PERIOD": 50,
                "FP_CORE_UTIL": 30,
        ...
        f.start(tag=f"benchmark_{chain}", overwrite=True, skip=skips)
        status[tup] = 'S'
    except FlowError as e:
        status[tup] = 'F'
    except Exception as e:
        with open(design_dir / "out.log", "w") as f:
            f.write(e)
        status[tup] = 'E'
    ...
    while True:
        ...
        try:
            for test, chain in status:
                ...
                screen.addstr(y, x + test_cell_width - 4, {'opt': '2', 'basic': '1', 'skip': '0'}[chain])
                screen.addstr(y, x + test_cell_width - 2, value, curses.color_pair({'N': 1, 'P': 2, 'S': 3, 'F': 4, 'E': 5}[value]))
                x += test_cell_width
            screen.refresh()
