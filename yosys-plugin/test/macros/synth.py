...
ys.run_pass(f"boundary_scan " + signal_flags, d)
ys.run_pass(f"write_verilog -noexpr -noattr top.after_bs.v", d)
ys.run_pass(f"synth", d)
ys.run_pass(f"autoname", d)
ys.run_pass(f"write_verilog -noexpr -noattr top.after_synth.v", d)
ys.run_pass(f"dfflibmap -liberty {lib}", d)
ys.run_pass(f"write_verilog -noexpr top.pre_techmap.v", d)
ys.run_pass(f"abc -liberty {lib}", d)
ys.run_pass(f"scan_replace -json_mapping {mapping}", d)
ys.run_pass(f"write_verilog top.nl.v", d)
